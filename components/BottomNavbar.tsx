        ...
        {MobOptions.map((opt, ind) => {
          return (
            <Link href={opt.goto} key={ind}>
              <AnimatePresence>
                <li
                  className={`flex flex-col gap-1 items-center justify-center hover:bg-secondary transition delay-100  ease-in cursor-pointer   rounded-lg px-4 py-2 m-2 ${
                    pathname === opt.goto && 'bg-secondary'
                  }`}
                  key={ind}
                >
                  <span>{opt.icon}</span>
                  {pathname === opt.goto && (null)}
                    <motion.span
                      className=" text-[10px]"
