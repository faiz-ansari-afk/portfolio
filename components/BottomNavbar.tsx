...
const BottomNavbar = () => {
  ...
  return (
    <div className='fixed  md:container  bottom-0  backdrop-blur-lg left-0 right-0 z-20 '>
        ...
        {Options.map((opt, ind) => {
          return (
            <Link href={opt.goto} key={ind}>
              <AnimatePresence>
              <li
                className={`flex  items-center justify-center my-4 mx-2 hover:bg-secondary transition delay-100  ease-in cursor-pointer p-3 border border-foreground rounded-lg ${
      ...
      <ul className="flex justify-between items-between hidden md:flex">
        {Options.map((opt, ind) => {
          return (
            <Link href={opt.goto} key={ind}>
              <AnimatePresence>
